...
\end{equation*}
\begin{proof}
By theorem \ref{thm:lambdaf} we can deduce
\begin{equation*}
    \sum_{n\leq x} \log n = \sum_{n\leq x} \sum_{d\vert n} \Lambda(d) =
\end{equation*}
\textcolor{red}{Hiffaispa mikä homma ääää}
\end{proof}
\end{lemma}
...
\end{equation*}
which is know as Mertens' second theorem. The final form follows from theorem 428 in Hardy-Wright
\begin{equation*}
    B = \gamma + \sum_{p\leq n} \left(\log \left(1-\frac{1}{p}\right)+\frac{1}{p}\right)\,,
\end{equation*}
proof of which is outside of the scope of this thesis.

Let us use the lemma \ref{lemma:mertenapu} with the sequence, where $c_p=\frac{\log p}{p}$ with prime indices and $c_n=0$ otherwise.

Now we have
\begin{equation*}
    C(x) = \sum_{p\leq x} \frac{\log p}{p} = \log x + O(1)\,.
\end{equation*}

\textcolor{red}{TODISTUS TOKALLE YHTÄSUURUUSMERKILLE PUUTTUU}

Now, with $f(t)=\frac{1}{\log t}$, as $x\rightarrow\infty$, we get
\begin{align*}
    ...
    & = 1 + \int_2^x \frac{dt}{t \log t} + \frac{O(1)}{\log x} + \int_2^x \frac{O(1)}{t \log^2 t}\,dt\\
    & = 1 + \log\log x - \log\log 2 + \frac{O(1)}{\log x} + \int_2^x \frac{O(1)}{t \log^2 t}\,dt
\end{align*}
