...
The totient function is also interesting as such, since its values differ quite unpredictable as it grows. That is why the growth rate of the Euler's totient function is the question, on which will be concentrating in this thesis.

To talk about the notation, the variables $k$, $m$, $n$ and $p$ denote usually natural numbers, $p$ being particularly a prime number. We define the set of natural numbers $\N$ to consist of positive integers, meaning $0 \not\in \N$. Also, in chapter \ref{apujutut} the notation $\lfloor x\rfloor$ is used to denote the integer part and $\{x\}$ the decimal part of a real number $x$. In other words, $x=\lfloor x\rfloor+\{x\}$.

It is also beneficial to go through the basic definitions concerning the essential areas before considering the totient function itself. Let us do just that.
...
In order to determine the order of growth of the totient function, we must introduce a few functions and auxiliary results that are used in a proof later on. Since all of the results of this chapter serve mainly as tools, we do not give details for some of them.

\subsection{Mertens' theorems}

The most important of the results is the Mertens' third theorem. We still need some lemmas before the proof of the theorem itself. First, we introduce few functions. The $O$-function, defined formally below, describes the relative size of two expressions.
...
It is sometimes useful to denote the partial sum of the $\Lambda$-function with another function, so called $\psi$-function. Along with it, let us also introduce another similar function, the $\vartheta$-function, as we will need it soon too.
...
Finally reaching the point, in which we are equipped to start dealing with the order of the totient function, let us still consider, why it is interesting at all. As pondered before, the Euler's product formula prsents the totient function in a more computable form than the mere definition. However, using it requires factorization of $n$, which still makes it difficult to estimate the size of $\phi(n)$ as $n$ gets bigger.
...
Hence, because there are infinitely many primes %(\textcolor{blue}{lähde?}),
\begin{equation*}
...
\begin{equation*}
    \lim_{t \rightarrow \infty} \left( 1-\frac{1}{t}\right)^\frac{t}{\log t} = \lim_{t \rightarrow \infty} \frac{(t-1)^\frac{t}{\log t}}{t^\frac{t}{\log t}}
\end{equation*}
