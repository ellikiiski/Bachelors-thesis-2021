...
Now that we have revised some of the basic concepts in number theory, let us introduce Euler's totient function itself. It is a number theoretic function describing with how many positive integers a given number has (or to be exact, has not) common divisors. We start by formally defining the function and then showing a few of its properties.
...
%That is, the value of the totient function at $n \in \N$ is the number of co-primes of $n$ smaller than it.
...
where $\prod_{p \,\vert\, n} \left(1 - \frac{1}{p}\right)$ means the product over distinct primes that divide $n$.
...
\section{Helpful functions and theorems}
\label{apujutut}
...
The most important of these are the Mertens' third theorem. However, we prove some lemmas before the proof of the theorem itself. First, we introduce $\Lambda$-function, sum of which has some useful properties keeping an eye on the Mertens' theorems.
...
\begin{proof}
To reach the form of Mertens' third theorem, it is shown in \textit{An Introduction to the Theory of Numbers} \cite{HardyWright} as theorem 428 (ch. 22.8, p. 466) that the constant
\begin{equation*}
...
\begin{definition}
The sigma function \emph{\cite{HardyWright}}
...
\begin{definition}
Chebyshev $\vartheta$-function \emph{\cite{HardyWright}}
